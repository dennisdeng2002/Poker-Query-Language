...
use crate::{Card, FlushingSuit, IsomorphicCard, IsomorphicHand, Suit};
...
impl SuitMap {
    ...
    /// Relabels `cards`' suits through `map` with sorted order.
    pub(crate) const fn iso_cards(self, cards: &[Card]) -> IsomorphicHand {
        match cards.len() {
            2 => {
                let mut arr = [self.iso_card(cards[0]), self.iso_card(cards[1])];
