use std::{fmt, hash::Hash, mem::transmute, ops, ops::Not};
...
impl Card64 {
    ...
    #[inline]
    pub(crate) const fn from_arr<const N: usize>(cards: [Card; N]) -> Self {
    ...
    pub(crate) const fn suits_with_min_count(self, n: CardCount) -> Suit4 {
        let mut res = Suit4::EMPTY;

        let mut i = 0;
        while i < Suit::ARR_ALL.len() {
            let suit = Suit::ARR_ALL[i];
            if self.count_by_suit(suit) >= n {
                res.set(suit);
            }
            i += 1;
        }
    ...
    pub const fn ranks_by_suit_count(self) -> [Rank16; 4] {
        ...
        unsafe {
            let [s, h, d, c]: [u16; 4] = transmute(self);
    ...
    pub const fn max_non_paired_rank(self) -> Option<Rank> {
        let [r1, r2, _, _] = self.ranks_by_suit_count();
